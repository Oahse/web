...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, token: Optional[str] = None) -> str:
        ...
        logger.info(f"New WebSocket connection: {connection_id}, User: {user_id}, Total: {len(self.connections)}")
    ...
    async def disconnect(self, connection_id: str, reason: str = "normal_closure"):
        ...
        logger.info(f"WebSocket disconnected: {connection_id}, User: {user_id}, Duration: {duration}, Reason: {reason}, Remaining: {len(self.connections)}")
    ...
    async def broadcast(self, message: str, exclude_connections: Optional[Set[str]] = None) -> int:
        ...
        connection_ids = list(self.connections.keys())  # Create copy to avoid modification during iteration
