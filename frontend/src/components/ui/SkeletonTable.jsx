...
export const SkeletonTable = ({
  ...
  const getCellWidth = (index) => {
    ...
    if (index === 0) return '25%'; // First column
    return `${75 / (actualColumns - (showActions ? 2 : 1))}%`;
  };
    ...
    <tr className="bg-background border-b border-border">
      {Array.from({ length: actualColumns }).map((_, index) => (
        <th key={index} className="px-4 py-3 text-left">
          <SkeletonText width={getCellWidth(index)} animation={animation} height="16px" />
        </th>
      ))}
    </tr>
  );

  const renderDataRow = (rowIndex) => (
    <tr
      key={rowIndex}
      className="border-b border-border-light last:border-b-0 hover:bg-background/50"
    >
      {Array.from({ length: actualColumns }).map((_, colIndex) => (
        <td key={colIndex} className="px-4 py-3">
          {showActions && colIndex === actualColumns - 1 ? (
            <div className="flex space-x-2">
              <SkeletonRectangle width="24px" height="24px" animation={animation} rounded="sm" />
              <SkeletonRectangle width="24px" height="24px" animation={animation} rounded="sm" />
            </div>
          ) : (
            <SkeletonText
  ...
  const renderDetailedRow = (rowIndex) => (
    <tr key={rowIndex} className="border-b border-border-light last:border-b-0">
      <td colSpan={actualColumns} className="px-4 py-4">
        <div className="flex items-center space-x-4">
          ...
          {showActions && (
            <div className="flex space-x-2">
              <SkeletonRectangle width="32px" height="32px" animation={animation} rounded="md" />
              <SkeletonRectangle width="32px" height="32px" animation={animation} rounded="md" />
            </div>
          )}
        </div>
  ...
  return (
    <div
      className={clsx(tableClasses, className)}
      role="status"
      ...
      <table className="w-full">
        ...
        <tbody>
          {Array.from({ length: rows }).map((_, index) =>
            variant === 'detailed' ? renderDetailedRow(index) : renderDataRow(index)
          )}
        </tbody>
      </table>
    </div>
...
export const SkeletonProductTable = (props) => (
  <SkeletonTable columns={5} {...props} />
);

export const SkeletonOrderTable = (props) => (
  <SkeletonTable columns={6} variant="detailed" {...props} />
);

export const SkeletonUserTable = (props) => (
