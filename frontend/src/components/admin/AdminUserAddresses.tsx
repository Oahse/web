...
export const AdminUserAddresses = ({ userId }) => { // Accept userId prop
  ...
  // Effect hook to fetch addresses on component mount or userId change
  useEffect(() => {
    if (userId) { // Fetch addresses only if userId is provided
      fetchAddresses(() => AdminAPI.getAddressesByUser(userId)); // Pass userId to AdminAPI call
    }
   ...
   * Calls the AdminAPI to perform the respective action and updates the local state.
  ...
  const handleAddressSubmit = async (e: any) => {
    ...
    try {
      if (editingAddressId) {
        ...
        const result = await AdminAPI.updateAddressForUser(userId, editingAddressId, formData); // Pass userId
        if (result?.data) {
        ...
        const result = await AdminAPI.createAddressForUser(userId, formData); // Pass userId
        if (result?.data) {
   ...
   * Calls the AdminAPI to delete the address and updates the local state.
   * @param {string} id - The ID of the address to delete.
   */
  const handleDeleteAddress = async (id: string) => {
    try {
      await AdminAPI.deleteAddressForUser(userId, id); // Pass userId
  ...
  if (loading) {
    return <div className="p-6 text-center text-copy-light">Loading user addresses...</div>;
  }
